---
title: 'PEC2_Anàlisi de dades òmiques: ANÀLISI RNA SEQ AMB R.'
...
# 1. Introducció.
En aquest anàlisi ens interessa fonamentalment l´expressió diferencial entre 3 grups, amb 10 mostres cadascun, obtingudes de 54 teixits de tiroides:
...
Partirem de dades preprocessades, és a dir, l´extracció de RNA, preparació de llibreries i la seqüenciació són qüestions que no tractarem. Tanmateix, aquestes seqüències ja han estat alineades amb un genoma de referència. La nostra feina comença a partir dels recomptes, i ens interessa estudiar si aquests difereixen significativament entre els diferents grups o condicions biològiques.
...
Treballarem doncs amb uns arxius "reduits" que només contenen 30 mostres:
```{r, include=F}
...
Una alternativa al plotMDS per examinar les relacions entre mostres és l’ús de cluster jeràrquic. Podem fer-ho mitjançant la funció heatmap.2 del paquet gplots, que calcula una matriu de distàncies euclidianes des del logCPM per als 500 gens més variables.
Farem servir el paquet RColorBrewer té esquemes de colors als quals s’accedeix mitjançant la funció brewer.pal. "RdYlBu" és una elecció habitual i "Spectral" també és agradable.
La funció png crearà un fitxer png per guardar les trames creades immediatament després, i tancarà aquest fitxer quan es truqui dev.off (). Per veure les parcel·les de manera interactiva, ometeu les dues línies.
...
dev.off()
```
```{r solutionChallenge3, fig.height=8, fig.width=6}
